...
def do_multi_query(key_code_list , server_array) : 
    ...
    '''
    print 'key_code_list' , key_code_list
    ...
    def do_query(key_code_list,machine) :
        try:
            ...
            s.sendline('cd ' + machine['remote_path'])
            s.prompt()
            s.sendline('tail -20 ' + log_code + '.log')
            s.prompt()
            content = s.before
            ...
            if end_index >= 0 :
                ...
                for i in range(len(key_code_list)) :
                    if content.find(key_code_list[i]) > 0 :
                        executing_id = i
                        break
...
def do_task(tasks) :
    ...
    return success_array , key_code_list

def get_request_result( success_array , key_code_list) :
    final_result = {}
    ...
    while query_result.empty() == False :
        ...
        final_result[machine['id']] = {}
        final_result[machine['id']]["host"] = machine['remote_host']
        if PID > 0 :
            final_result[machine['id']]["PID"] = PID
            final_result[machine['id']]["executing_id"] = executing_id
            final_result[machine['id']]["status"] = 1
        else :
            final_result[machine['id']]["executing_id"] = executing_id 
        if exitstatus == 0 :
            final_result[machine['id']]["status"] = 0
        else :
            if PID < 0 :
               final_result[machine['id']]["status"] = -1
